...
try {
    ...
    if ($data === null || !isset($data['cuerpoMensaje']) || !isset($data['perfilIAID']) || !isset($data['usuarioID'])) {
        $respuesta['error'] = 'Faltan datos necesarios para el registro del mensaje recibido';
    ...
    // Preparar la sentencia SQL para insertar el nuevo mensaje recibido
    $sql = "INSERT INTO MensajeRecibido (CuerpoMensaje, FechaGuardado, PerfilIAID, UsuarioID) VALUES (?, CURRENT_TIMESTAMP, ?, ?)";
    ...
    if ($stmt = mysqli_prepare($con, $sql)) {
        ...
        if(mysqli_stmt_execute($stmt)) {
            ...
            $respuesta['mensaje'] = 'Mensaje recibido guardado con éxito';
        } else {
            $respuesta['error'] = 'Error al guardar el mensaje recibido';
        }
        // Cerrar la sentencia
